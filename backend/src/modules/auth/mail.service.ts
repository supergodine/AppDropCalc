...
export class MailService {
  ...
  async sendPasswordRecovery(email: string, token: string): Promise<void> {
    ...
    // Configuração SendGrid via variáveis de ambiente
    ...
    const msg = {
      to: email,
      from: process.env.SENDGRID_FROM!, // Remetente verificado no SendGrid
    ...
    console.log(`[MailService] E-mail enviado para: ${email} via SendGrid.`);
  }
}
